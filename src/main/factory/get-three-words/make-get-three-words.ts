...
import { LevenshteinAlgorithm } from "@presentation/helpers";
...
export const makeGetThreeWordsControllerFactory = async ():Promise<GetThreeWordsController> => {
    ...
    const similarityAlgorithm = new LevenshteinAlgorithm(database)
    
    const getThreeWords = new GetThreeWordsController(validators, similarityAlgorithm)
